...
class Vehicle(object):
    ...
    def try_turn(self, s_states, wanted):
        for w in wanted:
            if w in s_states:
                return w
        return 'KL'

    def simple_safe_next_state(self, predictions):
        """
        This one is just to test that everything works as I expect it to
        """
        s_states = self.successor_states()
        new_state = 'KL'

        if self.goal_lane is not None:
            if self.goal_lane < self.lane:
                new_state = self.try_turn(s_states, ['LCR', 'PLCR'])
            elif self.goal_lane > self.lane:
                new_state = self.try_turn(s_states, ['LCL', 'PLCL'])
        
        traj = self.generate_trajectory(new_state, predictions)
...
#             print('for state ' + s + ' cost is ' + str(cost))
            if cost < best_cost:
                best_cost = cost
                best_state = s

        return self.generate_trajectory(best_state, predictions)
