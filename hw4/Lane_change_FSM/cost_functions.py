...
def goal_distance_cost(vehicle, trajectory, predictions, data):
    ...
    """
    lane_diff = abs(data.intended_lane - vehicle.goal_lane) + abs(data.final_lane - vehicle.goal_lane)
    dist = data.end_distance_to_goal

    return (lane_diff + 1) / (dist + 0.1)
...
def inefficiency_cost(vehicle, trajectory, predictions, data):
    ...
    """
    int_kin = vehicle.get_kinematics(predictions, data.intended_lane)
    fin_kin = vehicle.get_kinematics(predictions, data.final_lane)
    
    pred_time = 3

    int_s = int_kin[0] + int_kin[1] * pred_time + 0.5 * int_kin[2] * pred_time ** 2
    fin_s = fin_kin[0] + fin_kin[1] * pred_time + 0.5 * fin_kin[2] * pred_time ** 2
    
    diff = max(0, vehicle.goal_s - int_s) + max(0, vehicle.goal_s - fin_s)

    return diff / (2 * vehicle.goal_s + 1)
